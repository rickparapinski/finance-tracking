...
export default async function AccountsPage() {
  const [accounts, txSums] = await Promise.all([
    ...
    sql`SELECT account_id, SUM(amount) AS total FROM transactions GROUP BY account_id`,
  ]);

  const sumMap: Record<string, number> = {};
  for (const r of txSums) sumMap[r.account_id] = Number(r.total);

  const accountsWithBalance = accounts.map((a: any) => ({
    ...a,
    balance: Number(a.initial_balance) + (sumMap[a.id] ?? 0),
  }));
